...
def check_and_grant_achievements(user):
    """Проверяет все достижения пользователя и выдаёт недостающие."""
...
class UserDetailedStatisticsView(APIView):
    ...
    def get(self, request):
        user = request.user
        # Принудительно выдаём достижения
...
# ========== ОСТАЛЬНЫЕ КЛАССЫ (ExerciseTypeList, ExerciseConfigList и т.д.) ==========
# ... (они остаются без изменений, но для полноты я приведу их ниже) ...
...
class ExerciseSessionDetail(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if old_status != 'completed' and new_status == 'completed':
            ...
            # Дополнительно проверяем достижения (но основной метод уже в статистике)
            check_and_grant_achievements(user)
        else:
...
class UserProfileView(generics.RetrieveUpdateAPIView):
    serializer_class = UserProfileSerializer
    permission_classes = [permissions.IsAuthenticated]
    # parser_classes = [MultiPartParser, FormParser]  # добавляем
...
class NextQuestionView(APIView):
    ...
    def get(self, request):
        ...
        if last_element_key:
            elements = [e for e in elements if e['key'] != last_element_key]
            if not elements:
                elements = [e for e in elements]  # восстановить, если исключили все
