...
const ProfilePage = ({ onLogout }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        let ownConfigs;
        if (userRes.data.id) {
          ownConfigs = configsRes.data.filter(cfg => cfg.creator === userRes.data.id);
        } else {
          ownConfigs = configsRes.data.filter(cfg => cfg.creator_name === userRes.data.username);
        }
  ...
  const restartOnboarding = () => {
    if (user && user.id) {
      localStorage.removeItem(`onboardingCompleted_${user.id}`);
    }
    // Переходим на главную с параметром, чтобы запустить тур
    navigate('/?tour=1');
  };
    ...
    <div className="profile-container">
      <div className="profile-header">
        ...
        <div className="profile-actions">
          ...
          <button className="onboarding-btn" onClick={restartOnboarding}>
            <i className="fas fa-graduation-cap"></i> Пройти обучение
          </button>
