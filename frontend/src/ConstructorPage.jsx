...
const ConstructorPage = () => {
  ...
  const handleTypeToggle = (typeId) => {
    setFormData(prev => {
      ...
      // При добавлении нового типа инициализируем настройки по умолчанию
      if (!prev.exercise_type_ids.includes(typeId)) {
        const defaultSettings = { answer_mode: 'random' };
        if (typeId === 2) defaultSettings.intervals = ['Малая терция', 'Большая терция', 'Кварта', 'Квинта', 'Малая секста', 'Большая секста'];
        if (typeId === 3) defaultSettings.chords = ['Мажор', 'Минор', 'Увеличенный', 'Уменьшенный'];
        if (typeId === 1) defaultSettings.notes = 'all';
        setFormData(prev => ({
          ...prev,
          type_settings: {
            ...prev.type_settings,
            [typeId]: { ...prev.type_settings[typeId], ...defaultSettings }
          }
        }));
      }
  ...
  const intervalsCount = formData.type_settings[2]?.intervals?.length || 0;
  const chordsCount = formData.type_settings[3]?.chords?.length || 0;
      ...
      <form onSubmit={handleSubmit}>
        ...
        {formData.exercise_type_ids.includes(1) && (
          <>
            ...
            <div className="form-group">
              <label><i className="fas fa-gamepad"></i> Режим ответа для нот</label>
              <select
                value={formData.type_settings[1]?.answer_mode || 'random'}
                onChange={(e) => handleTypeSettingsChange(1, 'answer_mode', e.target.value)}
              >
                <option value="random">Случайный (пианино или варианты)</option>
                <option value="choice">Выбор из 4 вариантов (лёгкий)</option>
                <option value="piano">Пианино (сложный – нажать клавишу)</option>
              </select>
              <div className="helper-text">
                {formData.type_settings[1]?.answer_mode === 'choice' 
                  ? 'Будут предложены 4 варианта нот. Надо выбрать правильную.' 
                  : formData.type_settings[1]?.answer_mode === 'piano'
                    ? 'Нужно нажать на пианино ту же ноту, которая прозвучала.'
                    : 'Вопросы будут чередоваться: иногда выбор вариантов, иногда игра на пианино.'}
              </div>
            </div>
          </>
        )}
        ...
        {formData.exercise_type_ids.includes(2) && (
          <>
            <div className="form-group">
              <label><i className="fas fa-chart-line"></i> Какие интервалы включить?</label>
              <div className="checkbox-group-vertical">
            ...
            <div className="form-group">
              <label><i className="fas fa-gamepad"></i> Режим ответа для интервалов</label>
              <select
                value={formData.type_settings[2]?.answer_mode || 'random'}
                onChange={(e) => handleTypeSettingsChange(2, 'answer_mode', e.target.value)}
              >
                <option value="random">Случайный (пианино или варианты)</option>
                <option value="choice">Выбор из 4 вариантов (лёгкий)</option>
                <option value="piano">Пианино (сложный – нажать верхнюю ноту)</option>
              </select>
              <div className="helper-text">
                {formData.type_settings[2]?.answer_mode === 'choice' 
                  ? (intervalsCount === 1 
                      ? 'Выбран только один интервал. В вариантах будут конкретные ноты (например, "C-E", "D-F#").'
                      : `Выбрано ${intervalsCount} интервалов. В вариантах будут названия интервалов (например, "Малая терция", "Кварта").`)
                  : formData.type_settings[2]?.answer_mode === 'piano'
                    ? 'Прозвучит интервал, нужно нажать на пианино верхнюю ноту интервала.'
                    : 'Вопросы будут чередоваться: иногда выбор вариантов, иногда игра на пианино.'}
              </div>
            </div>
          </>
        ...
        {formData.exercise_type_ids.includes(3) && (
          <>
            <div className="form-group">
              <label><i className="fas fa-chord"></i> Какие типы аккордов включить?</label>
              <div className="checkbox-group-vertical">
            ...
            <div className="form-group">
              <label><i className="fas fa-gamepad"></i> Режим ответа для аккордов</label>
              <select
                value={formData.type_settings[3]?.answer_mode || 'random'}
                onChange={(e) => handleTypeSettingsChange(3, 'answer_mode', e.target.value)}
              >
                <option value="random">Случайный (выбор типа/конкретного аккорда или пианино)</option>
                <option value="choice">Выбор из 4 вариантов (лёгкий)</option>
                <option value="piano">Пианино (сложный – нажать тонику)</option>
              </select>
              <div className="helper-text">
                {formData.type_settings[3]?.answer_mode === 'choice'
                  ? (chordsCount === 1
                      ? 'Выбран только один тип аккорда. В вариантах будут конкретные аккорды (например, "C мажор", "D мажор").'
                      : `Выбрано ${chordsCount} типа аккордов. В вариантах будут типы (например, "Мажор", "Минор").`)
                  : formData.type_settings[3]?.answer_mode === 'piano'
                    ? 'Прозвучит аккорд, нужно нажать на пианино его тонику (основной звук).'
                    : 'Вопросы будут чередоваться: иногда выбор вариантов, иногда игра на пианино.'}
              </div>
            </div>
          </>
        ...
        <div className="form-group">
          <label><i className="fas fa-chart-simple"></i> Сложность (диапазон октав)</label>
